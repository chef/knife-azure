  ...
  context 'gem' do
    ...
    describe 'knife' , :if => is_config_present do
      ...
      context 'server' do
        ...
        context 'create Windows VM by using standard option and connect-to-existing-dns and over write winrm port ' do
          after(:each)  { run(delete_instance_cmd(@vm_name) + append_azure_creds(is_list_cmd = true) + " --yes --purge") }
          let(:command) { "knife azure server create --azure-vm-name #{@vm_name} --azure-dns-name #{@dns_name}" + append_azure_creds_for_windows + ' --azure-source-image "clogeny_win2k8_winrm_enabled"' + " --template-file " + get_windows_msi_template_file_path + " --server-url http://localhost:8889" + " --winrm-port 56892 --yes --azure-connect-to-existing-dns" }
          it 'should succeed' do
            match_status("should succeed")
          end
        end
