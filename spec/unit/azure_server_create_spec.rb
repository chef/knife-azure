...
describe Chef::Knife::AzureServerCreate do
  ...
  describe "parameter test:" do
    context "compulsory parameters" do
      it "azure_subscription_id" do
        @server_instance.config.delete(:azure_subscription_id)
        expect(@server_instance.ui).to receive(:error)
        expect { @server_instance.run }.to raise_error(SystemExit)
      end

      it "azure_mgmt_cert" do
        @server_instance.config.delete(:azure_mgmt_cert)
        expect(@server_instance.ui).to receive(:error)
        expect { @server_instance.run }.to raise_error(SystemExit)
      end

      it "azure_api_host_name" do
        @server_instance.config.delete(:azure_api_host_name)
        expect(@server_instance.ui).to receive(:error)
        expect { @server_instance.run }.to raise_error(SystemExit)
      end

      it "azure_source_image" do
        @server_instance.config.delete(:azure_source_image)
        expect(@server_instance.ui).to receive(:error)
        expect { @server_instance.run }.to raise_error(SystemExit)
      end

      it "azure_vm_size" do
        @server_instance.config.delete(:azure_vm_size)
        expect(@server_instance.ui).to receive(:error)
        expect { @server_instance.run }.to raise_error(SystemExit)
      end

      it "azure_service_location and azure_affinity_group not allowed" do
        @server_instance.config[:azure_affinity_group] = "test-affinity"
        expect(@server_instance.ui).to receive(:error)
        expect { @server_instance.run }.to raise_error(SystemExit)
      end

      it "azure_service_location or azure_affinity_group must be provided" do
        @server_instance.config.delete(:azure_service_location)
        expect(@server_instance.ui).to receive(:error)
        expect { @server_instance.run }.to raise_error(SystemExit)
      end

      it "raises an error if neither --azure-dns-name or --azure-vm-name are provided" do
        @server_instance.config.delete(:azure_dns_name)
        @server_instance.config.delete(:azure_vm_name)
        expect(@server_instance.ui).to receive(:error)
        expect { @server_instance.run }.to raise_error(SystemExit)
      end
      ...
      context "when winrm authentication protocol invalid" do
        it "raise error" do
          @server_instance.config[:winrm_auth_method] = "invalide"
          expect(@server_instance.ui).to receive(:error)
          expect { @server_instance.run }.to raise_error(SystemExit)
        end
      end
      ...
      context "when winrm-ssl ssl and missing thumbprint" do
        it "raise error if :winrm_no_verify_cert is not set" do
          @server_instance.config[:winrm_ssl] = true
          @server_instance.config[:winrm_no_verify_cert] = true
          expect(@server_instance.ui).to receive(:error)
          expect { @server_instance.run }.to raise_error(SystemExit)
        end
      end
    ...
    context "validate parameters" do
      ...
      it "raises error if invalid value is provided for daemon option" do
        ...
        @server_instance.config[:daemon] = "foo"
        @server_instance.config[:connection_protocol] = "cloud-api"
        expect { @server_instance.run }.to raise_error(
      ...
      it "does not raise error if daemon option value is 'none'" do
        allow(@server_instance).to receive(:is_image_windows?).and_return(true)
        @server_instance.config[:daemon] = "none"
        @server_instance.config[:connection_protocol] = "cloud-api"
        expect { @server_instance.run }.not_to raise_error(
          ArgumentError, "Invalid value for --daemon option. Use valid daemon values i.e 'none', 'service' and 'task'."
        )
      ...
      it "does not raise error if daemon option value is 'task'" do
        allow(@server_instance).to receive(:is_image_windows?).and_return(true)
        @server_instance.config[:daemon] = "task"
        @server_instance.config[:connection_protocol] = "cloud-api"
        expect { @server_instance.run }.not_to raise_error(
          ArgumentError, "Invalid value for --daemon option. Use valid daemon values i.e 'none', 'service' and 'task'."
        )
    ...
    context "server create options" do
      before do
        @server_instance.config[:connection_protocol] = "ssh"
        @server_instance.config[:connection_user] = "connection_user"
        @server_instance.config[:connection_password] = "connection_password"
        @server_instance.config.delete(:azure_vm_name)
        @server_instance.config.delete(:azure_storage_account)
      ...
      it "quick create" do
        ...
        @server_instance.config[:azure_dns_name] = "vmname" # service name to be used as vm name
        @server_instance.run
        ...
        test_params(testxml, @server_instance.config, @server_instance.config[:azure_dns_name],
      ...
      it "quick create with wirm - API check" do
        ...
        @server_instance.config[:azure_dns_name] = "vmname" # service name to be used as vm name
        @server_instance.config[:connection_user] = "opscodechef"
        @server_instance.config[:connection_password] = "Opscode123"
        @server_instance.config[:connection_protocol] = "winrm"
        @server_instance.run
        expect(@server_instance.config[:azure_vm_name]).to be == "vmname"
        testxml = Nokogiri::XML(@receivedXML)
      ...
      it "advanced create" do
        ...
        @server_instance.config[:azure_vm_name] = "vm002"
        @server_instance.config[:azure_storage_account] = "ka001testeurope"
        @server_instance.config[:azure_os_disk_name] = "os-disk"
        @server_instance.run
        testxml = Nokogiri::XML(@receivedXML)
        expect(xml_content(testxml, "MediaLink")).to be == "http://ka001testeurope.blob.core.windows-int.net/vhds/os-disk.vhd"
        expect(xml_content(testxml, "DiskName")).to be == @server_instance.config[:azure_os_disk_name]
        test_params(testxml, @server_instance.config, @server_instance.config[:azure_vm_name],
          @server_instance.config[:azure_vm_name])
      end
      ...
      it "create with availability set" do
        ...
        @server_instance.config[:azure_storage_account] = "ka001testeurope"
        @server_instance.config[:azure_os_disk_name] = "os-disk"
        @server_instance.config[:azure_availability_set] = "test-availability-set"
        @server_instance.run
        testxml = Nokogiri::XML(@receivedXML)
      ...
      it "server create with virtual network and subnet" do
        @server_instance.config[:azure_dns_name] = "vmname"
        @server_instance.config[:azure_network_name] = "test-network"
        @server_instance.config[:azure_subnet_name] = "test-subnet"
        @server_instance.run
        testxml = Nokogiri::XML(@receivedXML)
      ...
      context "Domain join:" do
        before do
          ...
          allow(@server_instance).to receive(:is_image_windows?).and_return(true)
          @server_instance.config[:connection_protocol] = "winrm"
          @server_instance.config[:connection_user] = "testuser"
          @server_instance.config[:connection_password] = "connection_password"
        end

        it "server create with domain join options" do
          @server_instance.config[:azure_domain_name] = "testad.com"
          @server_instance.config[:azure_domain_user] = "domainuser"
          @server_instance.config[:azure_domain_passwd] = "domainuserpass"
          @server_instance.run
          testxml = Nokogiri::XML(@receivedXML)
        ...
        it "server create with domain join options in user principal name (UPN) format (user@fully-qualified-DNS-domain)" do
          @server_instance.config[:azure_domain_user] = "domainuser@testad.com"
          @server_instance.config[:azure_domain_passwd] = "domainuserpass"
          @server_instance.run
          testxml = Nokogiri::XML(@receivedXML)
          expect(xml_content(testxml, "DomainJoin Credentials Domain")).to eq("testad.com")
        ...
        it 'server create with domain join options in fully-qualified-DNS-domain\\username format' do
          @server_instance.config[:azure_domain_user] = 'testad.com\\domainuser'
          @server_instance.config[:azure_domain_passwd] = "domainuserpass"
          @server_instance.run
          testxml = Nokogiri::XML(@receivedXML)
          expect(xml_content(testxml, "DomainJoin Credentials Domain")).to eq("testad.com")
        ...
        it "server create with domain join options including name of the organizational unit (OU) in which the computer account is created" do
          @server_instance.config[:azure_domain_user] = 'testad.com\\domainuser'
          @server_instance.config[:azure_domain_passwd] = "domainuserpass"
          @server_instance.config[:azure_domain_ou_dn] = "OU=HR,dc=opscode,dc=com"
          @server_instance.run
          testxml = Nokogiri::XML(@receivedXML)
          expect(xml_content(testxml, "DomainJoin Credentials Domain")).to eq("testad.com")
          expect(xml_content(testxml, "DomainJoin Credentials Username")).to eq("domainuser")
    ...
    context "when --azure-dns-name is not specified" do
      before(:each) do
        @server_instance.config[:azure_dns_name] = nil
        @server_instance.config[:azure_vm_name] = nil
      end
      ...
      it "generate unique dns name" do
        ...
        5.times do
          ...
          dns = @server_instance.send(:get_dns_name, @server_instance.config[:azure_dns_name])
      ...
      it "include vmname in dnsname if --azure-vm-name specified" do
        @server_instance.config[:azure_vm_name] = "vmname"
        dns = @server_instance.send(:get_dns_name, @server_instance.config[:azure_dns_name])
    ...
    context "connect to existing DNS tests" do
      before do
        @server_instance.config[:azure_connect_to_existing_dns] = true
      end

      it "should throw error when DNS does not exist" do
        @server_instance.config[:azure_dns_name] = "does-not-exist"
        @server_instance.config[:connection_user] = "azureuser"
        @server_instance.config[:connection_password] = "Jetstream123!"
        expect { @server_instance.run }.to raise_error(SystemExit)
      end
      ...
      it "port should be connection-port value specified in the option" do
        ...
        @server_instance.config[:connection_user] = "testuser"
        @server_instance.config[:connection_password] = "Jetstream123!"
        @server_instance.config[:connection_port] = "5990"
      ...
      it "extract user name when --connection-user contains domain name" do
        @server_instance.config[:connection_protocol] = "winrm"
        @server_instance.config[:connection_user] = 'domain\\testuser'
        @server_instance.config[:connection_password] = "Jetstream123!"
        @server_instance.config[:connection_port] = "5990"
        expect(@server_instance).to receive(:is_image_windows?).exactly(3).times.and_return(true)
        @server_params = @server_instance.create_server_def
      ...
      it "port should be connection-port value specified in the option" do
        ...
        @server_instance.config[:connection_user] = "azureuser"
        @server_instance.config[:connection_password] = "Jetstream123!"
        @server_instance.config[:connection_port] = "24"
        expect(@server_instance).to receive(:is_image_windows?).exactly(3).times.and_return(false)
        @server_params = @server_instance.create_server_def
      ...
      it "port should be 22 if user specified --connection-port 22" do
        ...
        @server_instance.config[:connection_user] = "azureuser"
        @server_instance.config[:connection_password] = "Jetstream123!"
        @server_instance.config[:connection_port] = "22"
        expect(@server_instance).to receive(:is_image_windows?).exactly(3).times.and_return(false)
        @server_params = @server_instance.create_server_def
      ...
      it "port should be 5985 if user specified --connection-port 5985" do
        @server_instance.config[:connection_user] = "azureuser"
        @server_instance.config[:connection_password] = "Jetstream123!"
        @server_instance.config[:connection_port] = "5985"
        @server_instance.config[:connection_protocol] = "winrm"
        expect(@server_instance).to receive(:is_image_windows?).exactly(3).times.and_return(true)
        @server_params = @server_instance.create_server_def
  ...
  describe "for connection protocol winrm:" do
    ...
    context "bootstrap node" do
      ...
      it "sets encrypted data bag secret parameter" do
        @server_instance.config[:encrypted_data_bag_secret] = "test_encrypted_data_bag_secret"
        expect(@server_instance).to receive(:is_image_windows?).exactly(3).times.and_return(true)
        @server_instance.run
      end

      it "sets encrypted data bag secret file parameter" do
        @server_instance.config[:encrypted_data_bag_secret_file] = "test_encrypted_data_bag_secret_file"
        expect(@server_instance).to receive(:is_image_windows?).exactly(3).times.and_return(true)
        @server_instance.run
      end

      it "sets winrm authentication protocol for windows vm" do
        @server_instance.config[:winrm_auth_method] = "negotiate"
        expect(@server_instance).to receive(:is_image_windows?).at_least(:twice).and_return(true)
        @server_instance.run
      end

      it "sets 'msi_url' correctly" do
        @server_instance.config[:msi_url] = "https://opscode-omnibus-packages.s3.amazonaws.com/windows/2008r2/x86_64/chef-client-12.3.0-1.msi"
        allow(@server_instance).to receive(:is_image_windows?).and_return(true)
        @server_instance.run
      end

      it "does not use 'install_as_service' anymore" do
        @server_instance.config[:install_as_service] = true
        allow(@server_instance).to receive(:is_image_windows?).and_return(true)
        @server_instance.run
      end
    end
  ...
  describe "for connection protocol ssh:" do
    ...
    context "linux instance" do
      ...
      context "ssh key" do
        before do
          @server_instance.config[:connection_password] = nil
          @server_instance.config[:ssh_identity_file] = "path_to_rsa_private_key"
        end
      ...
      context "bootstrap" do
        ...
        it "does not enable sudo password when connection_user is root" do
          @server_instance.config[:connection_user] = "root"
          expect { @server_instance.run }.not_to raise_error
        end

        it "sets secret parameter" do
          @server_instance.config[:encrypted_data_bag_secret] = "test_secret"
          expect { @server_instance.run }.not_to raise_error
        end

        it "sets secret file parameter" do
          @server_instance.config[:encrypted_data_bag_secret_file] = "test_secret_file"
          expect { @server_instance.run }.not_to raise_error
        end
        ...
        it "sets first_boot_attributes when json_attributes parameter specified" do
          @server_instance.config[:json_attributes] = '{"keyattr":"value"}'
          expect { @server_instance.run }.not_to raise_error
        end
      end
    end
  ...
  describe "for connection protocol cloud-api:" do
    before do
      @server_instance.config[:connection_protocol] = "cloud-api"
      allow(@server_instance).to receive(:msg_server_summary)
      @server_instance.config[:run_list] = ["getting-started"]
      @server_instance.config[:validation_client_name] = "testorg-validator"
      @server_instance.config[:chef_server_url] = "https://api.opscode.com/organizations/testorg"
    end

    after do
      @server_instance.config.delete(:connection_protocol)
      @server_instance.config.delete(:run_list)
      @server_instance.config.delete(:validation_client_name)
      @server_instance.config.delete(:chef_server_url)
    end
    ...
    context "linux instance" do
      ...
      it "check if all server params are set correctly" do
        ...
        expect(server_config[:chef_extension_version]).to eq("11.*")
        expect(server_config).to include(:chef_extension_public_param)
        expect(server_config[:chef_extension_public_param][:runlist]).to eq(@server_instance.config[:run_list].first.to_json)
        expect(server_config).to include(:chef_extension_private_param)
      end
    end
  ...
  describe "extended_logs feature for cloud-api connection protocol" do
    ...
    describe "fetch_chef_client_logs" do
      ...
      context "substatus found in server role response" do
        before do
          @server_instance.config[:azure_vm_name] = "vm04"
    ...
    describe "fetch_role" do
      context "role not found" do
        before do
          @server_instance.config[:azure_vm_name] = "vm09"
        end
      ...
      context "role found" do
        before do
          @server_instance.config[:azure_vm_name] = "vm01"
        end
    ...
    describe "fetch_extension" do
      context "Chef Extension not found" do
        context "other extension(s) available" do
          ...
          context "example-2" do
            before do
              @server_instance.config[:azure_vm_name] = "vm07"
              @role = fetch_role_from_xml
            end
        ...
        context "none of the extension available" do
          before do
            @server_instance.config[:azure_vm_name] = "vm06"
            @role = fetch_role_from_xml
          end
      ...
      context "Chef Extension found" do
        context "for Windows platform" do
          before do
            @server_instance.config[:azure_vm_name] = "vm02"
            @role = fetch_role_from_xml
          end
        ...
        context "for Linux platform" do
          before do
            @server_instance.config[:azure_vm_name] = "vm03"
            @role = fetch_role_from_xml
          end
