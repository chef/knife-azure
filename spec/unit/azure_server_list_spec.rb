...
describe Chef::Knife::AzureServerList do
  ...
  it "should display VM Name, Location, Provisioning State and OS Type for ARM command." do
    ...
    compute_mgmt_client = double("ComputeManagementClient")
    expect(@arm_server_instance.service).to receive(:compute_management_client).and_return(compute_mgmt_client)
    expect(compute_mgmt_client).to receive_message_chain(:virtual_machines, :list_all, :value!, :body, :value).and_return([])
