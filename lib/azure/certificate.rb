...
class Azure
  class Certificate
    ...
    def generate_public_key_certificate_data(params)
      ...
      ca.serial = Random.rand(65_534) + 1 # 2 digit byte range random number for better security aspect
    ...
    def generate_certificate(rsa_key, cert_params)
      @hostname = '*'
      unless cert_params[:domain].empty?
      ...
      cert.version = 2
      cert.serial = Random.rand(65_534) + 1 # 2 digit byte range random number for better security aspect
