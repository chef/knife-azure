...
1. Download the .publishsettings file from http://go.microsoft.com/fwlink/?LinkId=254432
2. Place it in the same directory with your knife.rb
3. Include this snippit within your knife.rb
...
# If there are multiple .publishsettings files
# This will only grab the first one
publishfile = File.expand_path(Dir.glob('*publishsettings').first)
publishxml = open(publishfile).read

# regex search out the interesting bits from the publishsettings xml
api_url = /(?<=Url=\")(?<url>.*)(?=\")/.match(publishxml)['url']
subscription_id = /(?<=Id=\")(?<id>.*)(?=\")/.match(publishxml)['id']
cert_base64 = /(?<=ManagementCertificate=\")(?<cert>.*)(?=\")/.match(publishxml)['cert']

# convert the base64 encoded pks12 cert into a der encoded .pem
cert_pks12 = Base64.decode64(cert_base64)
cert = OpenSSL::PKCS12.new cert_pks12

# use the same filename, but with .pem extension
pemfile=publishfile.sub(/publishsettings/,'pem')
open(pemfile,'wb') do |f|
  f.write cert.certificate.to_pem
  f.write cert.key.to_pem
end

# We pulled all these settings out of the first *.publishsettings file in the current_dir
knife[:azure_subscription_id]=subscription_id
knife[:azure_mgmt_cert]=pemfile # defaults to .chef/cacert.pem
knife[:azure_host_name]=URI(api_url).host
```
