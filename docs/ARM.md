...
By specifying the value `cloud-api` for the `bootstrap_protocol` option of `knife azurerm server create` instead of `winrm` or `ssh`, Microsoft Azure will install Chef Client using the `azure-chef-extension`. The process as a whole is asynchronous, so once the `knife azurerm server create` command has created the VM, full provisioning and Chef bootstrap will continue to occur even if the `knife` command is terminated before it completes.
