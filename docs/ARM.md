...
VMs created by `knife azurerm server create` use managed disks, so this option is
accepted for backward compatibility but no longer results in an
`Microsoft.Storage/storageAccounts` resource being created; it is not required and
has no effect on the resulting deployment.
```
-a, --azure-storage-account NAME Optional, unused. VMs use managed disks, so no storage
                                      account is created. This option is accepted only for
                                      backward compatibility and has no effect on the deployment.
...
User can pass `--azure-storage-account-type` option to set the storage account type
(SKU) of the VM's managed OS disk.
```
--azure-storage-account-type TYPE Optional. Sets the storage account type (SKU) of the VM's
                                      managed OS disk (case-sensitive). One of the following:
...
--azure-image-os-type centos
--azure-vm-size Standard_A1_v2
--azure-storage-account-type 'Premium_LRS'
-U myuser -P mypassword
```
